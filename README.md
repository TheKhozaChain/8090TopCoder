...
Generate interactive documentation with navigation from a directory of markdown files:

```bash
python3 documentation_generator.py input_directory -n "Project Name"
```

Or use it with the main script by adding the `--interactive-docs` flag:

```bash
python main.py --repo https://github.com/username/repo --interactive-docs
```
